...
if __name__ == '__main__':
    ...
    print('           -------------   开始识别网站   -------------')
    # 模型预测
    model = ClassificationModel('bert', 'model_weight', use_cuda=False)
    start = time.time()
    ...
    # data_df2['预测标签'] = predictions
    # data_df2.loc[data_df2['预测标签'] == 7,'预测标签'] = 5
    # print(data_df2)
    # data_df3 = data_df2[~data_df2['预测标签'].isin([7])]
    df5 = df1.loc[df1['status'] == 'False']
    df6 = df5['url']
    ...
    df6.to_excel(writer,sheet_name='爬虫失败的网址')
    writer.close()
