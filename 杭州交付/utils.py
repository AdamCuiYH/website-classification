...
@func_set_timeout(12)
def task_0(url,timeout):
    return request_spider_plus(url, timeout)
def request_spider(url,timeout):
    try:
        return task_0(url,timeout)
    except FunctionTimedOut:
        print("url_type_error")
        raise
def request_spider_plus(url, timeout):
    ''' Args:
               Enter url, wait time
...
def chrome_spider_son(url, SpiderTimeOut, SeleniumTimeOut):
    ...
    if  res :
        if float(res.status_code)//100==4 or float(res.status_code)//100==5:
            return [False,url,"No_Access",'','','']
        elif res.status_code == 200:
            if selenium_or_not(url, html)==True or get_content(url,html)[2]==0:
                return chrome_spyder_01(url, SeleniumTimeOut)
            else:
...
def chrome_spyder_01_plus(url, timeout):
    ''' Args:
...
def task(url,timeout):
    return chrome_spyder_01_plus(url, timeout)
def chrome_spyder_01(url,timeout):
    try:
        return task(url,timeout)
    except FunctionTimedOut:
...
def chrome_spyder(url):
    ''' Args:
           ...
           '''
    with open(r'爬虫文本.csv', 'a+', encoding='utf-8') as f1:
